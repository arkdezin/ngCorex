...
1. Reads `ngcorex.config.ts`
2. Resolves extends and applies presets
3. Reads `tokens.json`
4. Normalizes token values
5. Validates tokens
6. Generates CSS variables
7. Generates utilities (if enabled)
...
- Utilities are generated in a separate file if enabled

### Output formats

ngCorex supports multiple output formats:

- **css** (default) - Standard CSS custom properties
- **scss-variables** - SCSS variable syntax
- **scss-map** - SCSS map format

Configure output format in `ngcorex.config.ts`:

```typescript
export default defineConfig({
  output: {
    format: "scss-variables", // or 'css', 'scss-map'
  },
});
```
